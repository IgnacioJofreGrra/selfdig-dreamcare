    ...
    <>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">{(quizId as string).toUpperCase()}</span>
        </div>
        <p className="text-slate-700 dark:text-slate-300 mt-2">Interpretación: {result.interpretation}</p>
        {result.alertQ10 && (
